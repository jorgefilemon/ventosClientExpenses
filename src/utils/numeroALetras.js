export const numeroALetras = (function () {
  ...
  return function NumeroALetras(num, currency) {
    currency = currency || {};
    const mis_centavos = Math.round(num * 100) - Math.floor(num) * 100;
    let data = {
      numero: num,
      enteros: Math.floor(num),
      centavos: mis_centavos < 10 ? `0${mis_centavos}` : mis_centavos,
    ...
    if (data.enteros === 0)
      return (
        "CERO " + data.letrasMonedaPlural + " " + data.centavos + "/100 MN"
      );
    if (data.enteros === 1)
      return (
